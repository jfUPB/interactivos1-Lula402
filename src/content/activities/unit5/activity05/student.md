...
El ***DataView*** es como un traductor en este caso, porque nos permite ver que hay en ese buffer, pero de una manera que entendamos, por eso pusimos que ***Uint8Array***.  Antes de eso los bytes puritos ***(dataBytes).buffer*** los estabamos poniendo en un array buffer y eso lo estabamos guardando en la variable ***buffer***, luego el ***DataView*** es quien se encarga de traducirnos esos bytes a un entero de 8 bits.

El DataView es como un traductor en este caso, porque nos permite ver qué hay en ese buffer, pero aún no hemos dicho cómo lo vamos a leer. Antes de eso, los bytes puritos (dataBytes).buffer los estábamos poniendo en un array buffer y eso lo estábamos guardando en la variable buffer. Luego el DataView es quien se va a encargar de traducirnos esos bytes, pero todavía no lo ha hecho (eso está en lineas más abajo). Al decir Uint8Array, estamos diciendo que esos datos son bytes de 8 bits,y así están organizados en el buffer.

***- ¿Por qué es necesario hacer estas conversiones y no simplemente se toman tal cual los datos del buffer?***

Porque las conversiones son para saber como estamos hablando. Si yo tomo los datos puritos del buffer van a ser un montón de bits que no voy a saber ni como leerlos, mientras que si yo se que esos van a ser enteros, float, bool entonces los convierto y ahora si los puedo leer.
